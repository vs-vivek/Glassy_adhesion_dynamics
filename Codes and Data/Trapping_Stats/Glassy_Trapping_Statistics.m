...
%% Loop over 50 trajectories
num_trajectories = 50;
all_trap_times = [];

for i = 1:num_trajectories
    fprintf("Running trajectory %d/%d...\n", i, num_trajectories);
    [system, params] = run_glassy_model(params);  % full model
    tt1 = (1:params.num_steps) * params.dt;
    roff_change_idx = find(diff(system.roff_record) ~= 0);
    trap_times = diff(tt1(roff_change_idx));
...
title('Trapping Time Distribution (50 Glassy Model Trajectories)');
set(gca, 'FontSize', 14, 'LineWidth', 2);

%% Full Original Glassy Model Code (Wrapped in a Function)
function [system, params] = run_glassy_model(params)
    % Allocate space
